...
Go lecture notes and code exercises from the Central University backend course. 11 longreads covering the language from basics to HTTP services, plus hands-on snippets from *A Tour of Go*.
...
Runnable Go snippets from working through the official [A Tour of Go](https://go.dev/tour/), organized by topic in [`A_Tour_of_Go/`](A_Tour_of_Go/) — see [`A_Tour_of_Go/README.md`](A_Tour_of_Go/README.md) for the full folder-by-folder index (basics, flow control, structs/slices/maps, methods and interfaces, generics, concurrency).
